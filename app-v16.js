...
function saveRecords(next, sync = true) { localStorage.setItem(STORAGE_KEY, JSON.stringify(next)); render(); if (sync && sheetsAccessToken) syncRecordsToSheets(next).catch((error) => setSheetsStatus(error.message || 'シートへの保存に失敗しました。')); }
...
const sheetHeaders = { Meals: [['record_id', 'date', 'meal_type', 'time', 'item', 'updated_at']], Workouts: [['record_id', 'date', 'time', 'exercise', 'amount', 'unit', 'sets', 'updated_at']] };
const api = (path, options = {}) => fetch(`https://sheets.googleapis.com/v4/spreadsheets/${SPREADSHEET_ID}${path}`, { ...options, headers: { Authorization: `Bearer ${sheetsAccessToken}`, 'Content-Type': 'application/json', ...(options.headers || {}) } });
async function initializeSheet() {
  const response = await api('?fields=sheets.properties'); if (!response.ok) throw new Error('スプレッドシートを開けませんでした。');
  const data = await response.json(); const names = new Set((data.sheets || []).map((sheet) => sheet.properties.title));
  const requests = Object.keys(sheetHeaders).filter((name) => !names.has(name)).map((title) => ({ addSheet: { properties: { title } } }));
  if (requests.length) { const add = await api(':batchUpdate', { method: 'POST', body: JSON.stringify({ requests }) }); if (!add.ok) throw new Error('シートの初期化に失敗しました。'); }
  for (const [name, values] of Object.entries(sheetHeaders)) { const header = await api(`/values/${name}!A1?valueInputOption=RAW`, { method: 'PUT', body: JSON.stringify({ values }) }); if (!header.ok) throw new Error('シート見出しの作成に失敗しました。'); }
}
function recordsToSheetRows(all) {
  const meals = []; const workouts = [];
  all.forEach((record) => { if (record.type === 'meal') record.items.forEach((item) => meals.push([record.id, record.date, record.mealType, record.time, item, record.updatedAt])); else record.items.forEach((item) => workouts.push([record.id, record.date, record.time, item.name, item.amount, item.unit, item.sets, record.updatedAt])); });
  return { meals, workouts };
}
async function syncRecordsToSheets(all = records()) {
  setSheetsStatus('Googleスプレッドシートへ保存しています…'); await initializeSheet(); const { meals, workouts } = recordsToSheetRows(all);
  const clear = await api('/values:batchClear', { method: 'POST', body: JSON.stringify({ ranges: ['Meals!A2:F', 'Workouts!A2:H'] }) }); if (!clear.ok) throw new Error('古い記録を更新できませんでした。');
  const write = async (range, values) => { if (!values.length) return; const response = await api(`/values/${range}?valueInputOption=RAW`, { method: 'PUT', body: JSON.stringify({ values }) }); if (!response.ok) throw new Error('記録の保存に失敗しました。'); };
  await write('Meals!A2', meals); await write('Workouts!A2', workouts); setSheetsStatus('Googleスプレッドシートへ保存しました。');
}
async function loadFromSheets() {
  ...
  await initializeSheet(); const response = await api(`/values:batchGet?${ranges}`);
  ...
  if (!imported.length) { setSheetsStatus('シートに記録がありません。端末内の記録をシートへ保存します。'); await syncRecordsToSheets(); return; }
  if (!confirm('シートの内容で、端末内の記録を置き換えます。よろしいですか？')) { setSheetsStatus('読み込みを取り消しました。'); return; }
  saveRecords(imported, false); setSheetsStatus(`${imported.length}件の記録を読み込みました。`);
}
function connectSheets() {
  if (!window.google?.accounts?.oauth2) { setSheetsStatus('Googleログインの準備中です。少し待って再度押してください。'); return; }
  const client = google.accounts.oauth2.initTokenClient({ client_id: GOOGLE_CLIENT_ID, scope: 'https://www.googleapis.com/auth/spreadsheets', callback: async (token) => { try { sheetsAccessToken = token.access_token; await initializeSheet(); $('#connectSheetsButton').hidden = true; $('#loadSheetsButton').hidden = false; setSheetsStatus('接続しました。以後の保存・編集・削除はシートへ反映されます。'); await loadFromSheets(); } catch (error) { setSheetsStatus(error.message || '接続中にエラーが発生しました。'); } } });
...
$('#connectSheetsButton').onclick = connectSheets;
$('#loadSheetsButton').onclick = () => loadFromSheets().catch((error) => setSheetsStatus(error.message || '読み込み中にエラーが発生しました。'));
