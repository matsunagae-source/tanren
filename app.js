...
function renderMealHistory() {
  ...
  const renderDays = (root, dates, emptyText) => {
    ...
    groups.forEach((group) => {
      const card = document.createElement('article'); card.className = 'training-day';
      card.innerHTML = `<h3>${formatDate(group.date)}</h3><p class="training-summary">${group.items.map((meal) => `${meal.mealType}：${meal.items.join('、')}`).join('／')}</p>`;
      root.append(card);
    });
  };
  renderDays($('#todayMeals'), [today], '今日はまだ食事を記録していません。');
  renderDays($('#recentMeals'), Array.from({ length: 7 }, (_, index) => dateBefore(index + 1)), '最近7日間の食事記録はありません。');
}
